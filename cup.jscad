function getParameterDefinitions() {
  return [
      {name: 'width', type: 'float', initial: 95, caption: "取っ手の横幅"},
      {name: 'height', type: 'float', initial: 130, caption: "取っ手の縦幅"},
...
function main(p) {
    ...
    let totte = union(totte_sita,totte_ue,totte_tunagi).translate([-p.width/2-35,-p.hutosa/2,0]);
    return union(cylinder({r: 1, h: 10}),totte);
}